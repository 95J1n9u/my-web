...
const EditNotice = ({ notice, user, onSuccess, onCancel }) => {
    ...
    if (notice) {
      setFormData({
        ...
        expiresAt: notice.expiresAt ? 
          (notice.expiresAt.toDate ? notice.expiresAt.toDate() : new Date(notice.expiresAt))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await authService.updateNotice(notice.id, user.uid, updateData);

      if (result.success) {
        alert('공지사항이 성공적으로 수정되었습니다!');
        if (onSuccess) onSuccess(notice.id);
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">공지사항을 수정하세요</p>
        </div>
        <button
          onClick={onCancel}
